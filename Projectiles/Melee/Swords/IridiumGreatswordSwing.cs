...
public class IridiumGreatswordSwing : ModProjectile, ISyncedOnHitEffect
{
	...
	public override bool PreDraw(Player player, ref Color lightColor)
	{
		...
		{
			Main.EntitySpriteDraw(d with { color = Color.SandyBrown with { A = 0} * glowPercent });
			glowPercent = MathF.Sin(Utils.Remap(Projectile.localAI[2], 0, 15, MathHelper.PiOver2, MathHelper.Pi));
			Main.EntitySpriteDraw(d with { color = Color.SandyBrown with { A = 0 } * glowPercent, scale = d.scale * (1f + Projectile.localAI[2] * 0.03f)});
		}
		if (swinging)
		{
