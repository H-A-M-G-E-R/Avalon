...
public class TourmalineSapling : ModTile
{
    ...
    public static bool AttemptToGrowTourmalineFromSapling(int x, int y, bool underground)
	{
		...
		bool flag = AvalonModTree.GrowModdedTreeWithSettings(x, y, GemTree_Tourmaline);
    ...
    public static void GrowTourmalineTreeFXCheck(int x, int y)
    {
        ...
        if (treeHeight > 0)
        {
            ...
            {
                NetMessage.SendData(MessageID.SpecialFX, -1, -1, null, 1, x, y, treeHeight, ModContent.GoreType<TourmalineGemLeaves>());
            }
