...
public class ZirconSapling : ModTile
{
    ...
    public static bool AttemptToGrowZirconFromSapling(int x, int y, bool underground)
	{
		...
		bool flag = AvalonModTree.GrowModdedTreeWithSettings(x, y, GemTree_Zircon);
		if (flag && PlayerLOS(x, y))
		{
			GrowZirconTreeFXCheck(x, y);
		}
		return flag;
    }

    public static void GrowZirconTreeFXCheck(int x, int y)
    {
        int treeHeight = 1;
        ...
        if (treeHeight > 0)
        {
            ...
            {
                NetMessage.SendData(MessageID.SpecialFX, -1, -1, null, 1, x, y, treeHeight, ModContent.GoreType<ZirconGemLeaves>());
            }
            if (Main.netMode == NetmodeID.SinglePlayer)
            {
                WorldGen.TreeGrowFX(x, y, treeHeight, ModContent.GoreType<ZirconGemLeaves>());
            }
        }
    }
