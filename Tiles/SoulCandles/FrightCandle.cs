...
public class FrightCandle : ModTile
{
	...
	public override void MouseOver(int i, int j)
	{
		...
		player.cursorItemIconID = ModContent.ItemType<Items.Placeable.SoulCandles.FrightCandle>();
	}
	...
	public override void NearbyEffects(int i, int j, bool closer)
	{
		...
		{
			ModContent.GetInstance<BiomeTileCounts>().FrightCandleNearby = true;
		}
	}
	public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
	{
		...
		for (int k = 0; k < 7; k++)
		{
			...
			Main.spriteBatch.Draw(Mod.Assets.Request<Texture2D>("Tiles/SoulCandles/FrightCandle_Flame").Value, new Vector2(i * 16 - (int)Main.screenPosition.X + offsetX - (width - 16f) / 2f + x, j * 16 - (int)Main.screenPosition.Y + y) + zero, new Rectangle(frameX, frameY, width, height), color, 0f, default(Vector2), 1f, SpriteEffects.None, 0f);
		}
	}
}
