...
public class CursedFlamer : ModNPC
{
    ...
    public override void AI()
    {
        ...
        Vector2 mouthPosition = new Vector2(correctedCenter.X, NPC.position.Y + NPC.height * 1.26f - 6).RotatedBy(NPC.rotation,correctedCenter);
        if (!Main.rand.NextBool(4))
        {
            int dusty = Dust.NewDust(mouthPosition + Main.rand.NextVector2Circular(10f, 10f), 14, 12, DustID.CursedTorch, default, default, 100, new Color(), 2);
            Main.dust[dusty].noGravity = true;
            Main.dust[dusty].velocity.X += correctedCenter.DirectionTo(mouthPosition).X * Main.rand.NextFloat(2f);
            Main.dust[dusty].velocity.Y += correctedCenter.DirectionTo(mouthPosition).Y * Main.rand.NextFloat(2f);
        }
        if (Main.rand.NextBool(4))
        {
            int dusty2 = Dust.NewDust(mouthPosition + Main.rand.NextVector2Circular(10f, 10f), 14, 12, DustID.CursedTorch, default, default, 100, new Color(), 1f);
            Main.dust[dusty2].velocity.X += correctedCenter.DirectionTo(mouthPosition).X * Main.rand.NextFloat(2f);
            Main.dust[dusty2].velocity.Y += correctedCenter.DirectionTo(mouthPosition).Y * Main.rand.NextFloat(2f);
        }
        var num1179 = 0.7f;
        if (NPC.collideX)
        {
        ...
        if (Main.netMode != NetmodeID.MultiplayerClient && !Main.player[NPC.target].dead)
        {
            ...
            if (NPC.localAI[0] == 180f)
            {
                ...
                {
					Projectile.NewProjectile(NPC.GetSource_FromThis(), mouthPosition, NPC.Center.DirectionTo(Main.player[NPC.target].Center) * 7,ProjectileID.CursedFlameHostile,50,1f);
                }
                NPC.localAI[0] = 0f;
            }
        }
