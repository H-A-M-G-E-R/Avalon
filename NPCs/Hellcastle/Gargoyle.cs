...
public class Gargoyle : ModNPC
{
    ...
    public override void AI()
    {
        ...
        else if (!Main.player[NPC.target].dead)
        {
            ...
            float num365 = Math.Abs(NPC.position.X + NPC.width / 2 - (Main.player[NPC.target].position.X + Main.player[NPC.target].width / 2));
            float num366 = Main.player[NPC.target].position.Y - NPC.height / 2;
            if (num365 > 50f)
            {
                num366 -= 100f;
            }
