...
{
	private static Asset<Texture2D> Hat;
	public override void SetStaticDefaults()
    {
        Main.npcFrameCount[NPC.type] = 3;
        Data.Sets.NPCSets.Wicked[NPC.type] = true;
		NPCID.Sets.SpecificDebuffImmunity[Type][BuffID.Confused] = true;
		Hat = ModContent.Request<Texture2D>(Texture + "PartyHat");
    ...
    public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
    {
        ...
        {
            Rectangle frame = Hat.Frame(1, 6, 0, (int)NPC.ai[3]);
			Vector2 drawPos = NPC.Center + (new Vector2(16, -15 - (Frame * 2)) * NPC.scale).RotatedBy(NPC.rotation) - Main.screenPosition; //RotateAboutOrigin(NPC.position + new Vector2(28, -2) * NPC.scale, NPC.Center, NPC.rotation) - Main.screenPosition;

			Main.spriteBatch.Draw(Hat.Value, drawPos, frame, drawColor, NPC.rotation, NPC.frame.Size() / 2, NPC.scale, SpriteEffects.None, 0);
        }
    }
    ...
    public override void AI()
    {
        ...
        if (NPC.ai[2] > 230)
        {
            ...
            for (int i = 0; i < Main.rand.Next(3,6); i++)
            {
                ...
                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, ShootDirection, ModContent.ProjectileType<CougherCough>(), 21, 0, 255);
            }
        }
        ...
        NPC.rotation = MathHelper.Clamp(NPC.velocity.X * 0.1f, -maxRotate, maxRotate);
    }
        ...
        if (NPC.life <= 0 && Main.netMode != NetmodeID.Server)
        {
            if (DateTime.Now.Month == 2 && DateTime.Now.Day == 14)
            {
                for (int num673 = 0; num673 < 10; num673++)
                {
                    ...
                    int num675 = Dust.NewDust(new Vector2(NPC.position.X, NPC.position.Y), NPC.width, NPC.height, num674, (0f - NPC.velocity.X) * 0.3f, (0f - NPC.velocity.Y) * 0.3f, 0, default, 1.2f);
